...
export class RenderSystem {
  ...
  draw(
    ...
    // 4. Entity Rendering (Order Matters) with VIEW CULLING
    const margin = 200 / camera.zoom;
    const halfW = (width / 2) / camera.zoom + margin;
    const halfH = (height / 2) / camera.zoom + margin;

    const visibleEntities = entities.filter(e => 
        e.pos.x > camera.pos.x - halfW && e.pos.x < camera.pos.x + halfW &&
        e.pos.y > camera.pos.y - halfH && e.pos.y < camera.pos.y + halfH
    );
    
    visibleEntities.forEach(e => { if(e.type === EntityType.ZONE) this.worldRenderer.drawZone(e); });
    visibleEntities.forEach(e => { if(e.type === EntityType.WALL) this.worldRenderer.drawWall(e); });
    ...
    const tanks = visibleEntities.filter(e => e.type === EntityType.PLAYER || e.type === EntityType.ENEMY || e.type === EntityType.BOSS);
    tanks.forEach(e => {
    ...
    if (settings?.graphics.particles) {
        visibleEntities.forEach(e => { if(e.type === EntityType.PARTICLE) this.effectRenderer.drawParticle(e); });
    }

    this.ctx.globalCompositeOperation = 'source-over'; // Reset blending

    // UI Pass
    [...visibleEntities, player].forEach(e => {
