...
export class WorldSystem {
  ...
  static getSafeSpawnPos(entities: Entity[], gameMode: GameMode, teamId?: string): {x: number, y: number} {
      ...
      for (let i = 0; i < 50; i++) { // Increased attempts for better coverage
          const pos = { 
      ...
      // If no safe spot found, pick a random edge instead of center to feel different
      return { 
          x: Math.random() < 0.5 ? 500 : width - 500, 
