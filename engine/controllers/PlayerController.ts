...
export class PlayerController {
    ...
    update(dt: number, entities: Entity[], pushNotification: (msg: string, type?: any) => void) {
        ...
        // --- NEW: Cancel Invulnerability on Activity ---
        if (player.isInvulnerable) {
            const moveVec = this.inputManager.getMovementVector();
            const isFiring = this.inputManager.getIsFiring() || this.settings.gameplay.autoFire;
            const speed = Math.hypot(player.vel.x, player.vel.y);
            
            // If moving significanty or firing, cancel protection
            if (speed > 50 || isFiring || moveVec.x !== 0 || moveVec.y !== 0) {
                this.playerManager.cancelInvulnerability();
            }
        }
    }
