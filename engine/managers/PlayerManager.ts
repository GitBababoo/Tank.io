...
export class PlayerManager {
  ...
  spawnProtectionTimer: number = 0; // NEW: Timer for auto-expiring protection
  ...
  reset(spawnPos: {x: number, y: number}) {
      ...
      this.spawnProtectionTimer = 8.0; // Reset timer to 8 seconds
  ...
  setTeam(teamId: string | undefined) {
      this.entity.teamId = teamId;
      if (teamId) {
          this.entity.color = TEAM_COLORS[teamId as keyof typeof TEAM_COLORS] || COLORS.player;
      }
      this.emitUpdate();
  }
