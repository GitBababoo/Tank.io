...
export const Game: React.FC = () => {
  ...
  const handleStartOffline = (name: string, mode: GameMode, selectedClass: string) => {
    setPlayerName(name || 'Player');
    setGameMode(mode);
    setInitialClass(selectedClass);
    setIsHost(true); // Treat as host for local spawning logic
    setIsOffline(true);
    setGameState('CONNECTING');
  };
  ...
  useEffect(() => {
    if (gameState === 'PLAYING' && canvasRef.current) {
        ...
        setTimeout(() => {
            ...
            if (isOffline) {
                // Skip networking calls, engine will run in standalone mode
                console.log("[GAME] Starting in OFFLINE mode.");
            } else if (isHost) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-900 cursor-crosshair font-sans select-none touch-none">
      {gameState === 'LOBBY' && <LobbyView onStart={handleLobbyStart} onStartOffline={handleStartOffline} onOpenSettings={() => setShowSettings(true)} onOpenStudio={() => setGameState('STUDIO')} />}
      ...
      {gameState === 'PLAYING' && (
        <>
            ...
            {engineRef.current && (
                <IngameOverlay 
                    chatManager={engineRef.current.chatManager} 
