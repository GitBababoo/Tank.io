      ...
      <div className={`relative z-10 w-full max-w-7xl h-full md:h-[90vh] flex flex-col md:flex-row gap-6 p-4 md:p-8 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="w-full max-w-md p-4 bg-slate-900/50 border-l-4 border-green-500 rounded-r-xl backdrop-blur-sm flex items-center justify-between shadow-lg">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-black text-white">SYSTEM READY</div>
                    </div>
                </div>
                <div className="text-right">
                    <div className="text-[10px] text-green-400 font-bold uppercase">NO LAG</div>
                    <div className="text-[10px] text-slate-500 font-bold uppercase">Local Hosting</div>
                </div>
            </div>
        </div>
        ...
        <div className="w-full md:w-[480px] glass-panel rounded-3xl p-6 md:p-8 flex flex-col gap-6 shadow-2xl relative overflow-hidden shrink-0">
            <div className="space-y-2 mt-2">
                ...
                <input type="text" placeholder="YOUR NAME" className="w-full bg-slate-900/50 text-white border-2 border-slate-700 rounded-xl px-5 py-4 text-center font-bold text-lg outline-none focus:border-cyan-500 uppercase tracking-wider" value={name} onChange={(e) => setName(e.target.value)} maxLength={15} />
            </div>
            ...
            {lobbyMode === 'MAIN' ? (
                <>
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 gap-2 max-h-[200px] overflow-y-auto custom-scrollbar pr-1">
                            {GAME_MODES.map(mode => (
                                <button key={mode.id} onClick={() => setSelectedMode(mode.id)} className={`relative px-4 py-3 rounded-xl border transition-all flex items-center justify-between ${selectedMode === mode.id ? 'bg-slate-800 border-white text-white shadow-lg translate-x-1' : 'bg-slate-900/40 border-slate-800 text-slate-500 hover:bg-slate-800'}`} style={{ borderColor: selectedMode === mode.id ? mode.color : undefined }}>
                    ...
                    <div className="flex gap-2 mt-2">
                        <button onClick={handleHost} className="flex-1 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-black py-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"><Play fill="currentColor" size={20} /> CREATE ROOM</button>
                        <button onClick={() => setLobbyMode('JOIN')} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white font-black py-4 rounded-xl border-2 border-slate-600 hover:border-slate-500 transition-all active:scale-95 flex items-center justify-center gap-2"><Users size={20} /> JOIN FRIEND</button>
                    </div>
                </>
            ) : (
                <div className="space-y-4 animate-fade-in">
                    <div className="space-y-2">
                        ...
                        <input type="text" placeholder="PASTE ID HERE" className="w-full bg-slate-900/50 text-white border-2 border-green-700 rounded-xl px-5 py-4 text-center font-mono font-bold text-sm outline-none focus:border-green-500" value={targetRoomId} onChange={(e) => setTargetRoomId(e.target.value)} />
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => setLobbyMode('MAIN')} className="w-1/3 bg-slate-800 text-slate-400 font-bold py-4 rounded-xl hover:bg-slate-700">BACK</button>
                        <button onClick={handleJoin} className="flex-1 bg-green-600 hover:bg-green-500 text-white font-black py-4 rounded-xl shadow-lg transition-all active:scale-95">CONNECT</button>
                    </div>
                </div>
            )}
